...
// UiState = {
//   configured: boolean,
//   settings: { memberName, serverUrl, primaryWindow } (never the token),
...
1. **Onboarding** (when `!configured`): fields Server URL, Join token, Your name → Join
   button → `joinGroup`. Friendly error line under the form.
2. **Dashboard**: account header — one bar per available window ("Weekly", "5h") with
   ...
   Fair-share guide line at 100/N % on share bars. Dark-mode aware via
   `prefers-color-scheme`. No external fonts/assets.
